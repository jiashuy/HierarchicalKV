...
__global__ void tlp_update_kernel_with_io(
    ...
    uint32_t bucket_capacity, const uint32_t dim,
    const K* __restrict__ keys, const VecV* __restrict__ values,
    const S* __restrict__ scores, uint64_t n) {
  ...
  for (int offset = 0; offset < bucket_capacity + STRIDE; offset += STRIDE) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (result) {
        ...
        update_score<K, V, S>(bucket_keys_ptr, bucket_capacity, key_pos,
                              score);
...
__global__ void pipeline_update_kernel_with_io(
  ...
  __pipeline_memcpy_async(
      sm_digests[groupID] + rank * Load_LEN,
  ...
  for (int i = 0; i < loop_num; i++) {
    ...
    probing_digests = *reinterpret_cast<VecD_Comp*>(&sm_digests[groupID][
                          same_buf(i)  * BUCKET_SIZE + (GROUP_SIZE + rank) * Comp_LEN]);
    ...
    if (gt_vote == 0) {
      do {
        ...
        if (digest_idx >= 0) {
          ...
          sm_possible_pos[groupID][same_buf(i) * RESERVE + group_base] = key_pos;
          __pipeline_memcpy_async(
    ...
    if (rank < possible_num) {
      ...
      if (possible_key == target_key) {
        ...
        if (scores) {
          int key_idx_grid = blockIdx.x * blockDim.x + key_idx_block;
          const S* score_ptr = scores + key_idx_grid;
          CopyScore::ldg_sts(sm_scores[groupID] + diff_buf(loop_num), score_ptr);
        }
        auto key_ptr = BUCKET::keys(keys_ptr, key_pos);
  ...
  inline static void launch_kernel(Params& params, cudaStream_t& stream) {
    ...
    tlp_update_kernel_with_io<K, V, S, VecV, BLOCK_SIZE>
        <<<(params.n + BLOCK_SIZE - 1) / BLOCK_SIZE, BLOCK_SIZE, 0, stream>>>(
            ...
            params.dim, params.keys, reinterpret_cast<const VecV*>(params.values), 
  ...
  static void select_kernel(Params& params, cudaStream_t& stream) {
    ...
    auto launch_TLP = [&]() {
      if (total_value_size % sizeof(byte16) == 0) {
        using VecV = byte16;
        Launch_TLP_Update<K, V, S, VecV>::launch_kernel(params,
                                                          stream);
    ...
    auto launch_Pipeline = [&]() {
      if (total_value_size % sizeof(byte16) == 0) {
        using VecV = byte16;
        Launch_Pipeline_Update<K, V, S, VecV>::launch_kernel(params,
                                                            stream);
      } else if (total_value_size % sizeof(byte8) == 0) {
        using VecV = byte8;
        Launch_Pipeline_Update<K, V, S, VecV>::launch_kernel(params,
