...
template <typename K = uint64_t, typename V = byte4, typename S = uint64_t,
          uint32_t BLOCK_SIZE = 128, int Strategy = -1,
          typename EvictedObjectProcessor>
__global__ void find_or_insert_ptr_kernel_lock_key_v2(
    EvictedObjectProcessor processor, Bucket<K, V, S>* __restrict__ buckets,
    int32_t* __restrict__ buckets_size, const uint64_t buckets_num,
    uint32_t bucket_capacity, const uint32_t dim, const K* __restrict__ keys,
    V** __restrict__ value_ptrs, S* __restrict__ scores,
    K** __restrict__ key_ptrs, uint64_t n, bool* __restrict__ founds) {
  using BUCKET = Bucket<K, V, S>;
  using ScoreFunctor = ScoreFunctor<K, V, S, Strategy>;
  ...
  if (kv_idx < n) {
    key = keys[kv_idx];
    // at present, only support lfu, lru, customized.
    if (scores) { // lfu or customized
      score = scores[kv_idx];
    } else { // lru
      score = device_nano<S>();
    }
      ...
      if (result) {
        ...
        if (verify_score <= min_score) {
          ...
          if (expected_key == static_cast<K>(RECLAIM_KEY)) {
            ...
            occupy_result = OccupyResult::EVICT;
            auto evicted_value_ptr = bucket_values_ptr + key_pos * dim;
            processor.template operator()(expected_key, min_score, evicted_value_ptr);
          }
        } else {
          min_score_key->store(expected_key, cuda::std::memory_order_release);
        }
