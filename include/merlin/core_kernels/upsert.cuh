    ...
    if (occupy_result_cur == OccupyResult::INITIAL) {
      ...
      if (found_vote) {
        ...
        if (rank == i) {
          ...
          update_score<K, V, S>(bucket_keys_ptr, BUCKET_SIZE, key_pos, score);
        }
...
struct Launch_TLPv1_Upsert {
  ...
  inline static void launch_kernel(Params& params, cudaStream_t& stream) {
    ...
    tlp_v1_upsert_kernel_with_io<K, V, S, VecV, BLOCK_SIZE>
...
struct Launch_Pipeline_Upsert {
  ...
  inline static void launch_kernel(Params& params, cudaStream_t& stream) {
    ...
    pipeline_upsert_kernel_with_io<K, V, S, VecV, BLOCK_SIZE>
...
struct KernelSelector_Upsert<K, V, S, Sm80> {
  using ArchTag = Sm80;
  ...
  static void select_kernel(Params& params, cudaStream_t& stream) {
    ...
    auto launch_TLPv1 = [&]() {
      if (total_value_size % sizeof(byte16) == 0) {
        using VecV = byte16;
        Launch_TLPv1_Upsert<K, V, S, VecV>::launch_kernel(params, stream);
    ...
    auto launch_TLPv2 = [&]() {
      if (total_value_size % sizeof(byte16) == 0) {
        using VecV = byte16;
        Launch_TLPv2_Upsert<K, V, S, VecV>::launch_kernel(params, stream);
      } else if (total_value_size % sizeof(byte8) == 0) {
        using VecV = byte8;
        Launch_TLPv2_Upsert<K, V, S, VecV>::launch_kernel(params, stream);
      } else if (total_value_size % sizeof(byte4) == 0) {
    ...
    auto launch_Pipeline = [&]() {
      if (total_value_size % sizeof(byte16) == 0) {
        using VecV = byte16;
        Launch_Pipeline_Upsert<K, V, S, VecV>::launch_kernel(params, stream);
      } else if (total_value_size % sizeof(byte8) == 0) {
        using VecV = byte8;
        Launch_Pipeline_Upsert<K, V, S, VecV>::launch_kernel(params, stream);
      } else if (total_value_size % sizeof(byte4) == 0) {
...
__global__ void upsert_kernel_lock_key_hybrid(
    ...
    int* __restrict keys_index, uint64_t n) {
  using BUCKET = Bucket<K, V, S>;
